...
static void Draw_Color_Button(void *btn) {
  ...
  if (ptr->touch_flag == 0) {
    ...
    ILI9341_DrawRectangle(ptr->start_x, ptr->start_y, ptr->end_x - ptr->start_x,
...
static void Draw_Shape_Button(void *btn) {
  ...
  case LINE_WITH_CIRCLE:
    ...
    for (i = 0; i < ((ptr->end_x - ptr->start_x - 10) / 10); i++) {
      ...
      ILI9341_DrawCircle(ptr->start_x + 5 + i * 10,
                         ptr->start_y + (ptr->end_y - ptr->start_y) / 2, 3, 1);
    }
  ...
  case RUBBER:
    ...
    ILI9341_DrawRectangle(
        ptr->start_x + ((ptr->end_x - ptr->start_x - 24) / 2),
...
static void Command_Clear_Palette(void *btn) {
  LCD_SetColors(CL_WHITE, CL_WHITE);
  ILI9341_DrawRectangle(PALETTE_START_X, PALETTE_START_Y,

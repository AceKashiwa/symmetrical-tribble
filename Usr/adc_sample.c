...
void ADC_Sample_Once(uint32_t sample_rate_hz) {
  ...
  /* 阻塞等待整窗数据采集完成，加超时防止死循环 */
  {
    volatile uint32_t timeout = 0xFFFFFF;
    while (DMA_GetFlagStatus(DMA1_FLAG_TC1) == RESET && --timeout)
      ;
    if (timeout == 0)
      printf("[ADC] DMA timeout! sample_rate=%lu Hz\r\n", (unsigned long)sample_rate_hz);
  }
...
void ADC_Sample_GetChannel(uint8_t ch_idx, uint16_t **buf, uint32_t *count) {
  /* 双缓冲区轮转，避免连续两次调用时后一次覆盖前一次的返回指针 */
  static uint16_t tmp[2][ADC_SAMPLE_POINTS];
  static uint8_t toggle = 0;
  uint32_t i;
  uint8_t slot = toggle & 1;
  toggle ^= 1;

  for (i = 0; i < ADC_SAMPLE_POINTS; i++) {
    tmp[slot][i] = s_adc_buf[i * ADC_CH_NUM + ch_idx];
  }
  *buf = tmp[slot];
...
void ADC_Sample_DumpSerial(uint8_t print_raw_samples, uint16_t raw_count) {
  ...
  printf("NOTE: data is from the most recent ADC_Sample_Once call\r\n");
  ...
  if (print_raw_samples) {
    ...
    for (i = 0; i < raw_count; i++) {
      ...
      printf("%-5u %-7u %-7u %u\r\n", (unsigned)i,
             s_adc_buf[i * ADC_CH_NUM + 0], s_adc_buf[i * ADC_CH_NUM + 1],

...
static void ILI9341_GPIO_Config(void) {
  ...
  RCC_APB2PeriphClockCmd(
      /*控制信号*/
      ILI9341_CS_CLK | ILI9341_DC_CLK | ILI9341_WR_CLK | ILI9341_RD_CLK |
          ILI9341_BK_CLK | ILI9341_RST_CLK |
          /*数据信号*/
...
static void ILI9341_REG_Config(void) {
  ...
  if (lcdid == LCDID_ILI9341) {
    ...
    ILI9341_Write_Data(
        0x06); // 原来是0x02改为0x06可防止液晶显示白屏时有条纹的情况
...
void ILI9341_GramScan(uint8_t ucOption) {
  ...
  if (lcdid == LCDID_ILI9341) {
    ILI9341_Write_Data(
        0x08 | (ucOption << 5)); // 根据ucOption的值设置LCD参数，共0-7种模式
  } else if (lcdid == LCDID_ST7789V) {
    ILI9341_Write_Data(
        0x00 | (ucOption << 5)); // 根据ucOption的值设置LCD参数，共0-7种模式
  }
  ILI9341_Write_Cmd(CMD_SetCoordinateX);
 ...
 * @retval 无
 */
void ILI9341_OpenWindow(uint16_t usX, uint16_t usY, uint16_t usWidth,
                        uint16_t usHeight) {
 ...
 * @retval 无
 */
void ILI9341_Clear(uint16_t usX, uint16_t usY, uint16_t usWidth,
                   uint16_t usHeight) {
 ...
 * @retval 无
 */
void ILI9341_DrawLine(uint16_t usX1, uint16_t usY1, uint16_t usX2,
                      uint16_t usY2) {
  uint16_t us;
 ...
 * @retval 无
 */
void ILI9341_DrawCircle(uint16_t usX_Center, uint16_t usY_Center,
  ...
  while (sCurrentX <= sCurrentY) {
    ...
    if (ucFilled)
      for (sCountY = sCurrentX; sCountY <= sCurrentY; sCountY++) {
        ILI9341_SetPointPixel(usX_Center + sCurrentX,
                              usY_Center + sCountY); // 1，研究对象
...
void ILI9341_DispChar_EN(uint16_t usX, uint16_t usY, const char cChar) {
  ...
  // 设置显示窗口
  ILI9341_OpenWindow(usX, usY, LCD_Currentfonts->Width,
    ...
    uint8_t en_cn)      // 0为英文，1为中文
{
  ...
  out_ptr =
      (uint8_t
           *)&zoomBuff; // out_ptr没有正确传出，后面调用直接改成了全局变量指针！
...

{
  ...
  while (*ptr != '\0') {
    ...
    if (*ptr > 0x80) // 如果是中文
    {
      ...
      // 缩放字模数据，源字模为16*16
      ILI9341_zoomChar(WIDTH_CH_CHAR, HEIGHT_CH_CHAR, Charwidth, Font_Height,
                       (uint8_t *)&ucBuffer, psr, 1);
      // 显示单个字符
      ILI9341_DrawChar_Ex(x, y, Charwidth, Font_Height, (uint8_t *)&zoomBuff,
                          DrawModel);
      x += Charwidth;
      ptr += 2;
    } else {
      ...
      ILI9341_zoomChar(16, 24, Charwidth, Font_Height,
                       (uint8_t *)&Font16x24.table[Ascii * Font16x24.Height *
                                                   Font16x24.Width / 8],
                       psr, 0);
      // 显示单个字符
      ILI9341_DrawChar_Ex(x, y, Charwidth, Font_Height, (uint8_t *)&zoomBuff,
 ...
 * @retval None
 */
void LCD_SetTextColor(uint16_t Color) { CurrentTextColor = Color; }
 ...
 * @retval None
 */
void LCD_SetBackColor(uint16_t Color) { CurrentBackColor = Color; }
...
void LCD_ClearLine(uint16_t Line) {
  ILI9341_Clear(0, Line, LCD_X_LENGTH,
                ((sFONT *)LCD_GetFont())->Height); /* 清屏，显示全黑 */
}
/*********************end of file*************************/
